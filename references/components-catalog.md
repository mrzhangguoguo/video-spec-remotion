...
description: 视频内容类型词汇表 · 69 个标准内容类型 · Remotion 生态实现映射。
---

# 内容类型词汇表 · Components Catalog (Remotion)

69 个标准内容类型，拆分镜时每镜锚定一个组件 ID。
本目录同时描述「用途 / 何时用 / 内容期待」和 **Remotion 实现方案**。

[Remotion 生态依赖总览]
    核心框架:
    - remotion · @remotion/cli — 渲染引擎
    - @remotion/transitions — 转场效果（fade / wipe / slide / clock-wipe）
    - @remotion/motion-blur — 运动模糊

    动画:
    - framer-motion — React 动画库（spring / layout / AnimatePresence）
    - remotion 内置 spring() / interpolate() — 帧级精确控制

    图表:
    - @remotion/plot — Remotion 官方图表（line / bar / area / scatter / heatmap）
    - recharts — React 图表库（在 Remotion 中使用需包装 static）
    - @nivo/core + @nivo/bar, @nivo/line, @nivo/pie — 高质量 React 图表

    3D:
    - @remotion/three — Three.js in Remotion（3D 场景 / 模型 / shader）
    - @react-three/fiber + @react-three/drei — React Three.js 生态

    动效资产:
    - @remotion/lottie — After Effects Lottie 动画
    - @remotion/noise — Perlin noise / simplex noise（粒子、波纹）
    - @remotion/shader — WebGL fragment shader（自定义视觉效果）

    媒体:
    - @remotion/media-utils — 音频频谱分析 / 视频元数据
    - @remotion/captions — 字幕解析（SRT / WebVTT）

    UI / 代码:
    - lucide-react — 图标库（原 HyperFrames 的 Lucide 图标，直接复用）
    - prism-react-renderer — 代码语法高亮
    - @xyflow/react — 流程图 / 节点图（React Flow）

    插画:
    - SVG + Framer Motion — 矢量插画 + 动效
    - @remotion/lottie — AE 导出的 Lottie 动画


[使用方式]
    - 选组件：先看 [何时用] / [何时不用] 划范围，再看 [用途] 确认
    - 实现：看 [Remotion 实现] 字段，直接用对应库
    - 填内容：照 [内容期待] 把所需字段补齐
...
[aroll.subtitle-highlight] 字幕高亮 · Subtitle Highlight · SPOKEN-WORD CAPTIONS
    ...
    Remotion 实现: 自定义 React 组件 + spring() 逐词动画 + @remotion/captions 解析字幕文件。
...
[aroll.keyword-sticker] 关键词贴纸 · Keyword Sticker · POP-IN LABELS
    ...
    Remotion 实现: 自定义 React 组件 + spring() pop-in + SVG 引线动画。
...
[aroll.concept-card] 概念卡 · Concept Card · EXPLAINER CARD
    ...
    Remotion 实现: 自定义 React 卡片组件 + spring() slide-in + framer-motion layout animation。
...
[broll-hero.big-type] 大字海报 · Big Type · TYPOGRAPHIC POSTER
    ...
    Remotion 实现: 自定义 React 组件 + spring() slam/slide 入场 + CSS font-variation-settings 动态字重。
...
[broll-hero.big-number] 大数字 · Big Stat · STATISTIC HERO
    ...
    Remotion 实现: 自定义 React 组件 + interpolate() 数字滚动计数 + spring() pulse。
...
[broll-hero.pull-quote] 引用块 · Pull Quote · EDITORIAL MOMENT
    ...
    Remotion 实现: 自定义 React 组件 + spring() quote-mark 滑入 + interpolate() 作者打字机。
...
[broll-hero.inversion-flash] 反白闪屏 · Inversion Flash · CUT-IN TRANSITION
    ...
    Remotion 实现: 自定义 React 组件 + interpolate() 亮度反转（filter: invert）· 无需额外依赖。


[B-roll · 数据图表]
    所有图表组件统一使用 @remotion/plot 作为首选，recharts / @nivo 作为备选。
    @remotion/plot 优势：与 Remotion 帧系统原生集成，支持 interpolate 逐帧动画。
...
[broll-charts.line-chart] A1 · 折线图 · Line Chart · TREND OVER TIME
    ...
    Remotion 实现: @remotion/plot 的 <Line> + interpolate() 逐点绘制动画。
...
[broll-charts.multi-line] A2 · 多线对比 · Multi-line · MODEL COMPARISON
    ...
    Remotion 实现: @remotion/plot 多个 <Line> 叠加 + interpolate() 逐条淡入。
...
[broll-charts.bar-chart] A3 · 柱形图 · Bar Chart · DISCRETE QUANTITIES
    ...
    Remotion 实现: @remotion/plot 的 <Bar> + spring() 柱高生长动画。✅ 已实现。
...
[broll-charts.h-bar] A4 · 横向条形 · H-Bar · RANKING
    ...
    Remotion 实现: @remotion/plot 的水平 <Bar> 或 recharts <BarChart layout="vertical"> + spring()。
...
[broll-charts.stacked] A5 · 堆叠柱 · Stacked · COMPOSITION OVER TIME
    ...
    Remotion 实现: recharts <BarChart stackOffset="stack"> 或 @nivo/bar stacked 模式。
...
[broll-charts.area-chart] A6 · 面积图 · Area · ACCUMULATED VOLUME
    ...
    Remotion 实现: @remotion/plot 的 <Area> + interpolate() 渐变填充动画。
...
[broll-charts.donut] A7 · 环形图 · Donut · PROPORTION
    ...
    Remotion 实现: 自定义 SVG 环形 + interpolate() 圆弧绘制动画（stroke-dashoffset）· 或 @nivo/pie。
...
[broll-charts.scatter] A8 · 散点图 · Scatter · CORRELATION
    ...
    Remotion 实现: @remotion/plot 的 <Scatter> + spring() 逐点弹入。
...
[broll-charts.heatmap] A9 · 热力图 · Heatmap · 2D INTENSITY
    ...
    Remotion 实现: @remotion/plot 的 <Heatmap> 或自定义 CSS Grid + interpolate() 颜色过渡。
...
[broll-charts.gauge] A10 · 仪表盘 · Gauge · SINGLE METRIC
    ...
    Remotion 实现: 自定义 SVG 半圆弧 + interpolate() 弧度动画 + spring() 数字滚动。
...
[broll-charts.sparkline] A11 · 迷你图 · Sparkline · DENSE METRIC CARDS
    ...
    Remotion 实现: @remotion/plot 的迷你 <Line>（高度 40-60px）+ 自定义 KPI 卡片布局。
...
[broll-charts.sankey] A12 · Sankey 流图 · FLOW DISTRIBUTION
    ...
    Remotion 实现: d3-sankey + 自定义 SVG 渲染 + interpolate() 流条宽度动画。
...
[broll-abstract.analogy] 类比框 · Analogy · UNFAMILIAR ≈ FAMILIAR
    ...
    Remotion 实现: 自定义 React 组件 + spring() 左右滑入 + interpolate() 等号淡入。✅ 已实现。
...
[broll-abstract.black-box] 黑盒图 · Black Box · INPUT → ? → OUTPUT
    ...
    Remotion 实现: 自定义 React 组件 + spring() 输入→输出连线 + interpolate() "?"脉冲。
...
[broll-abstract.equation] 概念等式 · Concept Equation · A + B = C
    ...
    Remotion 实现: 自定义 React 组件 + spring() 逐项弹入 + "="符号 pulse。
...
[broll-abstract.spectrum] 光谱 · Spectrum · ONE AXIS · TWO POLES
    ...
    Remotion 实现: 自定义 SVG 轴 + interpolate() marker 位置动画 + spring() 标签弹出。
...
[broll-abstract.iceberg] 冰山 · Iceberg · VISIBLE / HIDDEN
    ...
    Remotion 实现: 自定义 SVG + interpolate() 水面以上淡入 + spring() 水面以下展开。
...
[broll-abstract.versus] 对照 · Versus · A vs B · DELTA
    ...
    Remotion 实现: 自定义 React 组件 + spring() 左右卡片滑入 + interpolate() vs 淡入。✅ 已实现。
...
[broll-abstract.placeholder] 占位框 · Placeholder · WHEN YOU LACK AN ASSET
    ...
    Remotion 实现: 自定义 React 占位框 + 虚线边框 + 文字说明。


[B-roll · 流程图]
    所有流程图组件统一使用 @xyflow/react (React Flow) 作为首选方案。
    React Flow 优势：节点 + 边的图结构，内置布局算法，动画支持好。
    备选：自定义 SVG + 手动布局（适用于简单流程）。
...
[broll-flows.complex] B1 · 复杂流程 · Multi-step · EXTENDED LINEAR FLOW
    ...
    Remotion 实现: @xyflow/react 节点图 + interpolate() 节点逐个点亮 + spring() 高亮 cluster。
...
[broll-flows.branching] B2 · 分支流程 · Branching · IF / ELSE
    ...
    Remotion 实现: @xyflow/react + 自定义分支节点 + spring() 路径高亮。
...
[broll-flows.decision-tree] B3 · 决策树 · Decision Tree · MULTI-LEVEL JUDGMENT
    ...
    Remotion 实现: @xyflow/react 层级布局 + interpolate() 推荐路径逐段高亮。
...
[broll-flows.state-machine] B4 · 状态机 · State Machine · STATES WITH TRANSITIONS
    ...
    Remotion 实现: @xyflow/react 自定义圆形节点 + spring() 状态切换动画 + SVG 自循环箭头。
...
[broll-flows.sequence] B5 · 时序图 · Sequence · API / INTERACTION TIMELINE
    ...
    Remotion 实现: 自定义 SVG 时序图 + interpolate() lifeline 从上到下绘制 + spring() 箭头弹出。
...
[broll-flows.swimlane] B6 · 泳道图 · Swimlane · MULTI-ROLE PROCESS
    ...
    Remotion 实现: 自定义 CSS Grid 泳道 + @xyflow/react 节点 + interpolate() 跨 lane 移交高亮。
...
[broll-flows.fork-join] B7 · 并行 / 汇合 · Fork-Join · PARALLEL EXECUTION
    ...
    Remotion 实现: @xyflow/react fork/join 布局 + spring() 并行节点同时弹入 + interpolate() merge 连线。
...
[broll-flows.loop] B8 · 循环流程 · Loop · ITERATIVE OPTIMIZATION
    ...
    Remotion 实现: 自定义 SVG 环形布局 + interpolate() 箭头沿环路循环 + spring() 退出条件淡入。


[B-roll · 结构图 I]
    结构图组件根据复杂度选择方案：
    - 简单（4-6 节点）：自定义 SVG + spring()
    - 复杂（7+ 节点）：@xyflow/react
...
[broll-structure.flow-chart] 流程图 · Flow Chart · LINEAR PROCESS
    ...
    Remotion 实现: 自定义 SVG 4 节点 + interpolate() 逐节点高亮推进。
...
[broll-structure.pyramid] 金字塔 · Pyramid · HIERARCHY
    ...
    Remotion 实现: 自定义 SVG 三角形 + spring() 从上到下逐层展开。
...
[broll-structure.funnel] 漏斗 · Funnel · CONVERSION
    ...
    Remotion 实现: 自定义 SVG 梯形 + spring() 逐层收缩 + interpolate() 最终留存强调。
...
[broll-structure.concentric] 同心圆 · Concentric · NESTED SCOPE
    ...
    Remotion 实现: 自定义 SVG 嵌套圆 + interpolate() 从外到内逐圈淡入 + spring() 核心强调。
...
[broll-structure.node-graph] 节点图 · Node Graph · ROUTING / WORKFLOW
    ...
    Remotion 实现: @xyflow/react + spring() 节点弹入 + interpolate() 边绘制动画。
...
[broll-structure.spectrum] 谱系图 · Spectrum · OPPOSITE AXIS
    ...
    Remotion 实现: 自定义 SVG 轴 + spring() marker 弹入 + interpolate() 标签淡入。
...
[broll-structures2.tree] C6 · 树 · Tree / Taxonomy · HIERARCHICAL CLASSIFICATION
    ...
    Remotion 实现: @xyflow/react 层级布局 + spring() 从根到叶逐层展开。
...
[broll-structures2.mind-map] C7 · 思维导图 · Mind Map · RADIAL DECOMPOSITION
    ...
    Remotion 实现: 自定义 SVG 径向布局 + spring() 分支逐条展开 + interpolate() 连线绘制。
...
[broll-structures2.matrix-2x2] C8 · 2x2 矩阵 · Matrix · POSITIONING / QUADRANTS
    ...
    Remotion 实现: 自定义 CSS Grid 4 象限 + spring() 对象逐个定位 + interpolate() 象限标签淡入。
...
[broll-structures2.venn] C9 · Venn 图 · INTERSECTION / UNION
    ...
    Remotion 实现: 自定义 SVG 三圆 + spring() 圆形弹入 + interpolate() 交集文字淡入。
...
[broll-structures2.layered-stack] C10 · 分层堆栈 · Layered Stack · ARCHITECTURE LAYERS
    ...
    Remotion 实现: 自定义 CSS Flexbox 堆叠 + spring() 从下到上逐层展开 + interpolate() focus 层高亮。
...
[broll-structures2.hub-spoke] C11 · Hub & Spoke · CENTRALIZED SYSTEM
    ...
    Remotion 实现: 自定义 SVG 径向布局 + spring() hub 中心弹入 + interpolate() spoke 逐条展开。
...
[broll-structures2.grid-map] C12 · 网格地图 · Grid Map · CLUSTER TOPOLOGY
    ...
    Remotion 实现: 自定义 CSS Grid + @remotion/noise 生成随机分布 + interpolate() 状态颜色过渡。
...
[broll-thinking.compare-table] D1 · 对比表 · Comparison Table · A VS B VS C
    ...
    Remotion 实现: 自定义 React 表格 + interpolate() 逐行淡入 + spring() 最优项高亮。
...
[broll-thinking.swot] D2 · SWOT 四宫格 · STRATEGIC ANALYSIS
    ...
    Remotion 实现: 自定义 CSS Grid 2×2 + spring() 象限逐个展开 + interpolate() 条目逐行淡入。
...
[broll-thinking.fishbone] D3 · 鱼骨图 · Fishbone · ROOT CAUSE ANALYSIS
    ...
    Remotion 实现: 自定义 SVG 鱼骨 + interpolate() 主干绘制 + spring() 成因逐条弹出。
...
[broll-thinking.timeline-row] D4 · 时间线 · Timeline · HISTORICAL EVOLUTION
    ...
    Remotion 实现: 自定义 SVG 时间轴 + interpolate() 节点从左到右逐个点亮 + spring() 卡片弹出。✅ 已实现。
...
[broll-thinking.gantt] D5 · 甘特图 · Gantt · PROJECT TIMELINE
    ...
    Remotion 实现: 自定义 CSS Grid + interpolate() 任务条从左到右生长 + spring() 里程碑标记。
...
[broll-thinking.kanban] D6 · Kanban 看板 · STATUS COLUMNS
    ...
    Remotion 实现: 自定义 CSS Flexbox 4 列 + spring() 卡片跨列移动动画。
...
[broll-thinking.card-grid] D7 · 卡片网格 · Card Grid · CONCEPT GALLERY
    ...
    Remotion 实现: 自定义 CSS Grid 4×2 + spring() 卡片逐个弹入 + interpolate() 推荐项 accent 高亮。


[B-roll · UI Mock]
    UI 模拟组件统一使用 React 组件直接渲染（本身就是 React，天然适配）。
    代码高亮使用 prism-react-renderer 或 shiki。
...
[broll-ui.terminal] 终端 · Terminal · CLI MOCK
    ...
    Remotion 实现: 自定义 React 终端组件 + interpolate() 逐字打字机 + spring() 光标闪烁。
...
[broll-ui.chat-thread] 对话流 · Chat Thread · LLM CONVERSATION
    ...
    Remotion 实现: 自定义 React 对话组件 + spring() 气泡逐条弹入 + interpolate() 流式打字效果。
...
[broll-ui.browser] 浏览器 · Browser · URL + VIEWPORT
    ...
    Remotion 实现: 自定义 React 浏览器框架 + spring() tab 切换 + interpolate() 页面滚动。
...
[broll-ui.code-editor] 代码编辑器 · Code Editor · SYNTAX HIGHLIGHTED
    ...
    Remotion 实现: prism-react-renderer 语法高亮 + 自定义 React 编辑器框架 + interpolate() 当前行高亮。
...
[broll-ui.api-call] API 调用 · Request / Response · REST · JSON
    ...
    Remotion 实现: 自定义 React 双面板 + prism-react-renderer JSON 高亮 + spring() 响应弹入。
...
[broll-ui.dashboard] 仪表盘 · Dashboard · LIVE METRICS
    ...
    Remotion 实现: 自定义 React 仪表盘 + @remotion/plot 迷你图 + spring() KPI 数字滚动。
...
[icons.lucide-set] I-2 · 常用图标库 · 48 个 · CURATED SET
    ...
    Remotion 实现: lucide-react 直接导入 + spring() 弹入动画。原 HyperFrames 的 Lucide 图标完全复用。
    可用图标（48 个，按组）：
...
[icons.stroke-weights] I-1 · 描边粗细 · 4 档 · STROKE WEIGHTS
    ...
    Remotion 实现: lucide-react 的 strokeWidth 属性，直接控制。
...
[illustrations.scene-thinking] 01 · 深度思考 · DEEP THINKING · SEATED + LIGHTBULB
    ...
    Remotion 实现: @remotion/lottie 加载 AE 导出的 Lottie 动画 + spring() 元素逐个出现。或 SVG + Framer Motion 手绘。
...
[illustrations.scene-co-create] 02 · 协作共创 · CO-CREATE · TWO PEEPS + SCREEN
    ...
    Remotion 实现: @remotion/lottie 或 SVG + Framer Motion。
...
[illustrations.scene-launch] 06 · 上线发布 · LAUNCH · WAVE + ROCKET TRAIL
    ...
    Remotion 实现: @remotion/lottie 或 SVG + Framer Motion + @remotion/noise 路径动画。
...
[illustrations.scene-library] I-3 · 场景插画库 · 6 SCENES · OPEN PEEPS STYLE
    ...
    Remotion 实现: 统一用 @remotion/lottie 加载 Lottie 动画资产。
...
```
要展示什么？
├── 数字 / 趋势 / 占比                → broll-charts.*（12 个）· 用 @remotion/plot
├── 步骤 / 决策 / 状态 / 协同         → broll-flows.*（8 个）· 用 @xyflow/react
├── 层级 / 分类 / 拓扑                → broll-structure.* (6) + broll-structures2.* (7)· 用 @xyflow/react 或自定义 SVG
├── 对比 / 分析 / 时间线              → broll-thinking.*（7 个）· 用自定义 React + spring()
├── 软件界面 / 终端 / 对话            → broll-ui.*（6 个）· 用自定义 React + prism-react-renderer
├── 抽象概念（没有具象图标）          → broll-abstract.*（7 个）· 用自定义 React + spring()
├── 重锤强调 / 大字 / 引用 / 反白闪屏 → broll-hero.*（4 个）· 用自定义 React + spring()
├── 出镜叠加（字幕 / 贴纸 / 概念卡）  → aroll.*（3 个）· 用自定义 React + @remotion/captions
├── 章节封面（手绘人物 + 道具）       → illustrations.*（7 个）· 用 @remotion/lottie 或 SVG
├── UI 小图标                         → icons.*（2 个）· 用 lucide-react
