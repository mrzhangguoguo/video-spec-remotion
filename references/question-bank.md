    ...
    [2.4 · 图形素材]
        ...
        [追问深化]
            ...
            **(c) 字体**
                ...
                - 用户说"我没有字体"→ 你回："等下选完主题就用主题自带字体（每个主题字体不同，预设由 themes/ 目录定义、自定义由项目根 theme.ts 定义）。除非你有强 brand 需求，否则跟主题走。"
...
[Phase 4 · 视觉主题选定 + 微调]
    ...
    [4.1 · 主题选择]
        ...
        [主问题]
            ...
            路径 1：8 个预设里挑一个
            路径 2：自己的自定义主题 —— 项目根目录的一个 theme.ts
        ...
        [路径 2 · 自定义主题]
            落地位置：项目根目录的 `theme.ts`（Remotion 渲染端唯一读取的主题文件，
            路径基准 = video-spec.md 所在目录）。

            两种入口：
            (a) 已有设计文件 → 用户把自己的 theme.ts（DesignTokens 格式）放项目根 / 粘贴聊天
            (b) 只有感觉没有具体值 → 用户描述给你（三个形容词/参考链接/类似品牌），
                你上网调研后给 2-3 个候选方案，定稿后在项目根生成 theme.ts
